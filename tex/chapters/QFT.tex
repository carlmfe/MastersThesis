...
\end{TODO}
\phpar[Introduce gauge theory in the path integral formalism and go through
    Yang-Mills theory with gauge fixing]
...
\equiv A_\mu^a(x) T^a\)\margincomment{It should probably be noted that
    \(\mathcal{A}_\mu\) is a real-valued field.} which transforms according to
\begin{equation}
...
\phpar[Introduce the \emph{relevant} fields of the Standard Model and its
    construction.
    Maybe mention the Higgs mechanism?]

\section{Loop Integrals and Regularisation}
\phpar[Introduce loop integrals, how to calculate them, where divergences
    appear and how to regularise them.]
\begin{TODO}
