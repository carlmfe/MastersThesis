...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \State{\(U \gets V^\ast\)}
  \end{algorithmic}
  \caption{Diagonalisation step on an \(n\by n\) symmetric matrix \(A\) to find a matrix \(U\) s.t.\ \(A = U^T \begin{bmatrix} \sigma & \vec{0} \\ \vec{0} & A^\prime \end{bmatrix} U\), where \(\sigma = \sqrt{|\lambda|}\) for some eigenvector \(\lambda\) of \(AA^\ast\) and \(A^\prime\) is an \((n-1) \by (n-1)\) symmetric matrix.
    The algorithm relies on a function eigh(\(M\)) to give an eigenvalue with its corresponding eigenvector of a hermitian matrix \(M\), and GramSchmidt(\(M\)) to orthogonalise a complex, invertible matrix \(M\). The algorithm relies on some machine precision parameter \(\epsilon\).}
