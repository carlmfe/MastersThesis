...
All the calculations have been done with self-produced Mathematica~\cite{Mathematica} scripts, with double-checks done with hand-calculations and comparison to existing results~\cite{Debove:2010kf,Demirci:2014gva}.
...
We assign the \(N\) scalar products \(m^2_i\) to the invariant masses of the incoming and outgoing particles, thereby not counting them as kinematic degrees of freedom, which leaves us with \(\frac{N(N-3)}{2}\) degrees of freedom.\footnote{I note that we
...
Doing Passerino-Veltman reduction from \cref{qft:subsec:pave} using the \verb|FeynCalc|~\cite{FeynCalc1,FeynCalc2,FeynCalc3,FeynCalc4} package, we can rewrite this to
\begin{equation}
...
Working it out in with \verb|FeynCalc|, we get
\begin{align}
  \nonumber
...
where
\begin{align}
  \tilde{C}^{\prime\prime}_Z = 2\sum_{AB} \pclosed{R^{\tilde{q}}_{A1}(R^{\tilde{q}}_{B1})^\ast C_{qqZ}^L + R^{\tilde{q}}_{A2}(R^{\tilde{q}}_{B2})^\ast C_{qqZ}^R}^2 \Re{C_{00}(0,\hat{s},0,m_A^2,m_{\tilde{g}}^2,m_B^2)}.
\end{align}
...
The cancellation of UV divergences between \(\d[Q^2]{\hat\sigma_v}\) and \(\d[Q^2]{\hat\sigma_{c.t.}}\) has been confirmed symbolically in Mathematica using analytic expressions for the Passarino-Veltman functions implemented in the \verb|FeynCalc| package \verb|Package-X|~\cite{PackageX}.
...
Again using analytical expressions for the Passarino-Veltman functions from \verb|Package-X|, it can be shown that the \(\frac{1}{\epsilon^2}\)-term above cancels with when added together with the cross-section \cref{pc:eq:sigma_g_virtual}.
...
The remaining divergences are \emph{collinear} divergences, arising from the limits where \(y \to 0, 1\), or \(\cos\theta \to \pm 1\), meaning the radiated gluons are collinear with the parton emitting them.
The means for removing these divergences is different, and involve redifing the parton distribution functions in a manner similar to the renormalisation of the fields and couplings of the bare Lagrangian in \cref{qft:sec:renormalisation}.
...
\begin{align}
  ...
  \biggr\}u(k_i).
\end{align}
Again evaluating in Mathematica using the \verb|FeynCalc| package, the squared matrix element becomes, after averaging over the 2 initial quark spins and \(N_C\) colours, and 2 initial gluon polarisations and \(N_C^2-1\) gluon states,
\begin{align}
  \nonumber
