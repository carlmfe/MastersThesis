...
In this chapter, I will go through the details of the calculation of the leading order (LO) contributions to the cross-section of production of pairs of electroweakinos at the level of partons, i.e.\ the fundamental particles of QCD\@.
...
\begin{align}
  ...
  \M_{\tilde{g}} = & i2\delta_{ab} C_F \epsilon_\mu^\ast(q) \mu^{4-d} g_s^2 g \sum_{AB} (C_{qqZ}^L (R_{A1}^{\tilde{q}})^\ast R_{B1}^{\tilde{q}} + C_{qqZ}^R (R_{A2}^{\tilde{q}})^\ast R_{B2}^{\tilde{q}}) \\
                   & \times \loopintegral[d]{\ell} \frac{\bar{v}_2 N_{AB} (2\ell-k_i+k_j)\^\mu u_1}{(\ell^2-m_{\tilde{g}}^2) ((\ell-k_i)^2 - m_A^2)((\ell+k_j)^2 - m_B^2)},
\end{align}
where
\begin{equation}
...
\begin{align}
  \nonumber
  \tilde{V}^{\prime\prime}_Z = & \frac{2}{(C_{qqZ}^L)^2 + (C_{qqZ}^R)^2}\sum_{AB} \pclosed{R^{\tilde{q}}_{A1}(R^{\tilde{q}}_{B1})^\ast C_{qqZ}^L + R^{\tilde{q}}_{A2}(R^{\tilde{q}}_{B2})^\ast C_{qqZ}^R}^2 \\
                               & \times\Re{C_{00}(0,0,\hat{s},m_A^2,m_{\tilde{g}}^2,m_B^2)}.
\end{align}
...
% we get
We can \(z^{\epsilon} = 1 + \epsilon \ln z + O(\epsilon^2)\) and the \emph{plus distribution} to write
\begin{equation}
  (1-z)^{1-2\epsilon} = -\frac{1}{2\epsilon} \delta(1-z) + \plusdist{\frac{1}{1-z}} -2\epsilon \plusdist{\frac{\ln(1-z)}{1-z}} + O(\epsilon^2).
\end{equation}
Plus distributions are defined such that for an arbitrary function \(g(z)\) and a plus distribution \(f_+(z)\)
\begin{equation}
  \plusdist{\frac{f(z)}{1-z}} \equiv \frac{f(z)}{1-z} - \delta(1 - z) \integral[_0^z]{y} \frac{f(y)}{1-y}
\end{equation}
Using this, we can rewrite \cref{pc:eq:gluon_emission_phase_space_integral} as
\begin{align}
  \nonumber
